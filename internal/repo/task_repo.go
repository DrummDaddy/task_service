...
func (r *TaskRepo) Create(ctx context.Context, t models.Task) (uint64, error) {
...
func (r *TaskRepo) Get(ctx context.Context, id uint64) (models.Task, error) {
	var t models.Task
...
func (r *TaskRepo) UpdateWithHistory(ctx context.Context, taskID uint64, upd TaskUpdate, changedBy uint64) (models.Task, error) {
	tx, err := r.db.BeginTx(ctx, nil)
	if err != nil {
		return models.Task{}, fmt.Errorf("begin tx: %w", err)
	}
	defer func() {
		_ = tx.Rollback()
	}()
...
`, taskID).Scan(&cur.ID, &cur.TeamID, &cur.Title, &curDesc, &cur.Status, &curAssignee, &cur.CreatedBy, &cur.CreatedAt, &cur.UpdatedAt)
	if err != nil {
		...
		return models.Task{}, fmt.Errorf("task lock: %w", err)
	}
	if curDesc.Valid {
	...
	history := make([]models.TaskHistoryItem, 0, 4)
	addHistory := func(field string, oldV, newV *string) {
		history = append(history, models.TaskHistoryItem{
			TaskID:    taskID,
			ChangedBy: changedBy,
			FieldName: field,
			OldValue:  oldV,
	...
	if setClauses != "" {
		...
		if _, err := tx.ExecContext(ctx, "UPDATE tasks SET"+setClauses+"WHERE id =?", args...); err != nil {
			return models.Task{}, fmt.Errorf("task update: %w", err)
		}
...
`, h.TaskID, h.ChangedBy, h.FieldName, h.OldValue, h.NewValue)
			if err != nil {
				return models.Task{}, fmt.Errorf("task_history insert: %w", err)
			}
		}
	}
...
`, taskID).Scan(&out.ID, &out.TeamID, &out.Title, &outDesc, &out.Status, &outAssignee, &out.CreatedBy, &out.CreatedAt, &out.UpdatedAt)
	if err != nil {
		return models.Task{}, fmt.Errorf("task selected updated: %w", err)
	}
	if outDesc.Valid {
	...
	if err := tx.Commit(); err != nil {
		return models.Task{}, fmt.Errorf("commit tx: %w", err)
...
func (r *TaskRepo) History(ctx context.Context, taskID uint64, limit int, offset int) ([]models.TaskHistoryItem, error) {
...
`, taskID, limit, offset)
	...
	defer rows.Close()
	var out []models.TaskHistoryItem
	for rows.Next() {
		var h models.TaskHistoryItem
...
func (r *TaskRepo) ListComments(ctx context.Context, taskID uint64, limit int, offset int) ([]models.TaskComment, error) {
	if limit < 0 || limit > 200 {
		limit = 50
	}
	if offset < 0 {
